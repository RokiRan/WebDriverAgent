...
//    [[FBRoute POST:@"/wda/clipboard/setText"] NSStringFromSelector(@selector(handleSetClipboardText:))],
    [[FBRoute POST:@"setClipboardText"].withoutSession respondWithTarget:self action:@selector(handleSetCopyData:)],
  ];
}
...
// 新建的一个用于操作剪贴板的接口
+ (id<FBResponsePayload>)handleSetCopyData:(FBRouteRequest *)request
{
  if (nil != FBSession.activeSession) {
    [FBSession.activeSession kill];
  }
  
  NSString *value = [request.arguments[@"value"] description];
  UIPasteboard *pasteboard = [UIPasteboard generalPasteboard];
  pasteboard.string = value;
  NSLog(@"Value parameter: %@", request.arguments[@"value"]);
  return FBResponseWithOK();
}
